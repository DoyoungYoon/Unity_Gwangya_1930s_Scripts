...
public class Enemy_Eye : MonoBehaviour {
    ...
    float degree = 60;
    float intervalDegree = 0.1f;
    float beginOffsetDegree = 60;
    float radius = 30;
    ...
    void DrawEye()
    {
        if (enemyAI.aiState == Enemy_AI.AI_State.Fight)
        {
            radius = 0;
        }
        else
        ...
        while (i < count)
        {
            ...
            Vector3 chkPosition = new Vector3(beginCos * tempRadius, 0 , beginSin * tempRadius);
            chkPosition = transform.rotation * chkPosition;
            RaycastHit hit;
            if (Physics.Raycast(transform.position, chkPosition, out hit, LayerMask.NameToLayer("Player")))
            {
